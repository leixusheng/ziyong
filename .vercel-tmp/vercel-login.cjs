...
function openBrowser(url) {
  ...
  if (!urlPattern.test(url)) { log(`Error: Invalid URL pattern`); return; }
...
async function doLogin() {
  log('Starting login authorization...'); log('');
  const loginPid = startBackgroundLogin();
  log('Waiting for authorization URL...');
  const authUrl = await waitForAuthUrl();
  if (authUrl) {
    log('========================================'); log('Authorization URL extracted'); log(`vercel login is running in background (PID: ${loginPid})`); log('Opening browser for authorization...'); log('========================================'); log('');
    openBrowser(authUrl);
    console.log(JSON.stringify({ status: 'needs_auth', auth_url: authUrl, log_file: LOG_FILE }));
  } else {
    log('Failed to get authorization URL'); process.exit(1);
  }
}
async function main() {
